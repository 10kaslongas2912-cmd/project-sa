...
export const SponsorshipProvider: React.FC<{
  ...
  const hydrated = { ...defaultState, ...loadPersisted(dogId ?? null) };
  const [state, dispatch] = useReducer(reducer, {
    ...hydrated,
    dogId: dogId ?? hydrated.dogId ?? null,
  });
  ...
  useEffect(() => {
    ...
    if (dogId !== state.dogId) {
      // เคลียร์ state เก่า + ไม่ใช้ค่าเก่าที่ persist ไว้ข้ามน้องหมา
      dispatch({ type: "RESET" });
  ...
  const actions = useMemo<SponsorshipActions>(
    ...
    [setDog, setPlan, setFrequency, setAmount, setPaymentMethod, reset, validateForForm, validateForPayment, validateForFormDraft]
  );
...
export function useSponsorshipActions() {
  ...
  if (!ctx)
    throw new Error("useSponsorshipActions must be used within SponsorshipProvider");
  return ctx;
}
